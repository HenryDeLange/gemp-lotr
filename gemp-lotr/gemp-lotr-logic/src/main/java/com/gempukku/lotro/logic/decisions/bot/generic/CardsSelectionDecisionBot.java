...
public class CardsSelectionDecisionBot extends MakeBotDecision {
    ...
    public String getBotChoice(LotroGame game, AwaitingDecision awaitingDecision) {
        ...
        if (numberOfCardsToSelect > 0 && cardId.length > 0) {
            ...
            // Handle skirmish strength modifier selection
            if (choice.isEmpty()) {
                if (game.getGameState().getCurrentPhase() == Phase.SKIRMISH && game.getGameState().getSkirmish() != null) {
                    // Assume the target would be the character currently in the skirmish (if possible)
                    Set<PhysicalCard> minions = game.getGameState().getSkirmish().getShadowCharacters();
                    Set<Integer> minionCardIds = new HashSet<>();
                    for (PhysicalCard minion : minions)
                        minionCardIds.add(minion.getCardId());
                    for (String tempCardId : cardId) {
                        int availableCardId = Integer.parseInt(tempCardId);
                        if (availableCardId == game.getGameState().getSkirmish().getFellowshipCharacter().getCardId()
                                || minionCardIds.contains(availableCardId)) {
                            choice = Integer.toString(availableCardId);
                            try {
                                LOG.trace(" SELECT: Select the skirmishing character "
                                        + "(" + decision.getSelectedCardById(availableCardId).getBlueprint().getTitle() + ")");
                            }
